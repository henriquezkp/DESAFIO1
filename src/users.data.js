...
/*
let novaQuery = usersList.filter(f => f.age > 66).map(m => {
    return {
        nome: m.name,
        idade: m.age,
        login: m.login.username
    }
});
...
// Desestruturação
outraQuery = usersList.filter(f => f.age > 66).map(m => {
    return {
        nome: m.name,
        idade: m.age,
        login: m.login.username
...
const { nome, idade, login } = outraQuery;
console.log(nome);
console.log(idade);
console.log(login);*/